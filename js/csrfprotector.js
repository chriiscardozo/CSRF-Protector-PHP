...
//===============================================================
// Função genérica para adicionar o parâmetro em um form
//===============================================================
function addFieldToForm(form, type, name, value){
...
function csrfprotector_init() {
	...
	//=======================================
	// Adicionando código que injeta campo CSRFP.CSRFP_TOKEN no Form, pois o método seguinte não funcionou para os Forms do Reports
	//=======================================
	for(var i = 0; i < document.forms.length; i++) {
		addFieldToForm(document.forms[i], 'hidden', CSRFP.CSRFP_TOKEN, CSRFP._getAuthKey());
	}
	...
	for (var i = 0; i < document.links.length; i++) {
        document.links[i].addEventListener("mousedown", function(event) {
						// <i> não tem href . Corrigir depois!
					  if(event.target.href === undefined || event.target.href === null) return;
...
$('form').live({
	keypress: function(){ if($(this).find('input[type=hidden][name='+CSRFP.CSRFP_TOKEN+'],').length == 0) addFieldToForm($(this).get(0), 'hidden', CSRFP.CSRFP_TOKEN, CSRFP._getAuthKey()); },
	mouseover: function(){ if($(this).find('input[type=hidden][name='+CSRFP.CSRFP_TOKEN+'],').length == 0) addFieldToForm($(this).get(0), 'hidden', CSRFP.CSRFP_TOKEN, CSRFP._getAuthKey()); }
});
